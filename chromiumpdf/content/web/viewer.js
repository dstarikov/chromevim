...
class PDFPresentationMode {
  ...
  _touchSwipe(evt) {
    console.log('got a touch event: ' + evt);
    if (!this.active) {
      return;
    }
  ...
  _addWindowListeners() {
    ...
    window.addEventListener('touchend', this.touchSwipeBind, {passive: false});
  }
...
// Distance for zoom step
const STEP_DIST = 25;
const MIN_STEP_DIST = 10;
function touchZoomHandle(evt) {
  switch (evt.type) {
    case 'touchstart':
    ...
    case 'touchmove':
      if (evt.touches.length === 2) {
        this.endtouches = evt.touches;
        const d = PinchGestureDist(this.endtouches) - PinchGestureDist(this.starttouches);
        console.log('touchmove with distance ' + d);
        let previousScale = PDFViewerApplication.currentScale;

        if (d < 0) {
          PDFViewerApplication.zoomOut((-d/100));
        } else if (d > 0) {
          PDFViewerApplication.zoomIn((d/100));
        }

        let currentScale = PDFViewerApplication.currentScale;
        if (previousScale !== currentScale) {
          let scaleCorrectionFactor = currentScale / previousScale - 1;
          let rect = PDFViewerApplication.container.getBoundingClientRect();
          let dx = evt.clientX - rect.left;
          let dy = evt.clientY - rect.top;
          PDFViewerApplication.container.scrollLeft += dx * scaleCorrectionFactor;
          PDFViewerApplication.container.scrollTop += dy * scaleCorrectionFactor;
        }
      ...
      break;
    case 'touchend':
      if (evt.touches.length === 0 && !!this.starttouches && !!this.endtouches) {
        const d = PinchGestureDist(this.endtouches) - PinchGestureDist(this.starttouches);
        console.log('toucend with distance ' + d);
        if (Math.abs(d) >= MIN_STEP_DIST) {
          let previousScale = PDFViewerApplication.currentScale;
...
document.addEventListener('touchstart', touchZoomHandle.bind(this), {passive: false});
document.addEventListener('touchmove', touchZoomHandle.bind(this), {passive: false});
document.addEventListener('touchend', touchZoomHandle.bind(this), {passive: false});
*/
